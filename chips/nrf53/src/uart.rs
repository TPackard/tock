...
impl<'a> Uarte<'a> {
    ...
    fn set_baud_rate(&self, baud_rate: u32) {
        ...
        match baud_rate {
            ...
            31250 => regs.baudrate.set(0x00800000),
    ...
    fn enable_rx_interrupts(&self) {
        let regs = &*self.registers;
        regs.inten.write(Interrupt::ENDRX::SET);
    }

    fn enable_tx_interrupts(&self) {
        let regs = &*self.registers;
        regs.inten.write(Interrupt::ENDTX::SET);
    }

    fn disable_rx_interrupts(&self) {
        let regs = &*self.registers;
        regs.inten.write(Interrupt::ENDRX::CLEAR);
    }

    fn disable_tx_interrupts(&self) {
        let regs = &*self.registers;
        regs.inten.write(Interrupt::ENDTX::CLEAR);
    }
